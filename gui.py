...
class HomeworkCheckerApp:
    ...
    def setup_ui(self):
        ...
        ttk.Entry(batch_frame, textvariable=self.batch_parent_var, width=50).grid(row=0, column=1, sticky=(tk.W, tk.E), padx=(5,0), pady=5)
        ttk.Button(batch_frame, text="浏览", command=self.browse_batch_parent, width=8).grid(row=0, column=2, padx=(5,5))
        ...
        # 批量检查说明
        help_label = ttk.Label(batch_frame, text="💡 功能：自动扫描‘母文件夹’下所有子文件夹，生成一份汇总Excel，显示每个学生在每个实验的提交情况。",
                              foreground="gray")
...
class EditFormatWindow:
    ...
    def setup_ui(self):
        ...
        ttk.Entry(main_frame, textvariable=self.name_var, width=40).grid(row=0, column=1, sticky=(tk.W, tk.E), pady=(0, 10))
    ...
    def save_format(self):
        ...
        if not is_folder and "{扩展名}" not in template:
            if not messagebox.askyesno("提示", "这是一个文件格式，但模板中没有包含 {扩展名} 变量。\n文件可能无法正常打开。确定继续吗？"):
                return
